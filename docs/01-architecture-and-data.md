...
- **`hostService.js` gained `createHost`/`deleteHost` (2026-09-22)** — hosts were read-only before (seed-only, no way to add/remove one anywhere in the app). Same mutation shape as every other write: `maybeFail()`, `persist()`, and (for delete) a demo-only simplification — it doesn't cascade to visitors/invites that reference the removed host by id, they just fall back to the `host?.name ?? '—'` pattern already used everywhere a host lookup can miss. A real system would block or reassign first.
