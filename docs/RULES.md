...
- [ ] Dashboard (`/app/*`) screens: check **both** light and dark — the toggle is real there (`ThemeToggle`, scoped to `AppShell`'s own root, see `03-design-system.md`). Marketing/auth screens (`/`, `/login`): **light only, no toggle**, ever — don't add one.
- [ ] Uses `.site-container` for section width, not a one-off `max-w-*`, and is full-width (no `max-w-2xl`-style centered wrapper) unless it's genuinely a narrow form like the login card.
...
- [ ] Logo is the real provided artwork (`public/full-logo.png` / `public/logo-icon.png`), never re-derived as styled text — full lockup in spacious/light spots, icon-only where space is tight or the surface is dark. See `03-design-system.md`.
...
Mock-only backend · Redux Toolkit · Axios · `react-icons` app-wide (incl. inside shadcn internals) · illustrations from unDraw/Storyset, recoloured · no gradients, ever · dense type scale · dashboard-only dark/light toggle (marketing stays light) · `/login` exists but doesn't gate `/app/*` — there is no `/signup`, accounts aren't a real concept in this demo (2026-09-22) · logo is real provided artwork (`full-logo.png`/`logo-icon.png`), not styled text. Full detail: `00-prd.md` § Decisions. Anything genuinely new that comes up goes through a chat confirmation first, then gets added here.
